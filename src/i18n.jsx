...
export const STRINGS = {
  en: {
    ...
    tabs: { overview: "Overview", cashflow: "Cash Flow", networth: "Net Worth", invest: "Investments & Goals", retire: "Retirement" },
    list: { empty: "No items yet", name: "Name", amount: "Amount", add: "Add" },
    kNetWorth: "Net Worth",
    netWorthSub: (a, d) => `Assets ${a} − Debt ${d}`,
    kSavingsRate: "Savings Rate",
    srExcellent: "Excellent ✦", srHealthy: "Healthy", srImprove: "Could improve", srOver: "Spending over income",
    kMonthlySurplus: "Monthly Surplus",
    surplusSub: (inc, exp) => `Income ${inc} / Spend ${exp}`,
    ...
    noGoals: 'No goals yet → add them under "Investments & Goals"',
    ...
    secBudgetActual: "Budget vs Actual",
    budgetActualSub: "Recurring expenses as budget, vs this month's actual spend (incl. variable)",
    budgetActualMeta: (actual, budget) => `Actual ${actual} / Budget ${budget}`,
    budgetNone: "No recurring expenses set",
    over: (v) => `${v} over`, left: (v) => `${v} left`,
    ...
    investNote: "Note: holdings here are a current snapshot you update manually. They are tracked separately from the Net Worth tab — if you also list investments there, avoid double-counting. This tool only tracks; investment decisions are your own or for a professional to advise on.",
    ...
    qDone: "Done!",
    qProgress: (step, total) => `Question ${step} / ${total}`,
    namePH: "Name (e.g. Car loan)",
    amountPH: "Type or say the amount",
    addThis: "Add this one",
    listening: "Listening… tap to stop", micSay: "🎤 Say it",
    heard: (t) => `Heard: "${t}"`,
    back: "Back", skip: "Skip", finish: "Finish", next: "Next",
    allSet: "All set ✦",
  ...
  "zh-TW": {
    ...
    goalNoSurplus: "本月無結餘",
    investNote: "提醒:這裡的投資金額是「現況快照」,你需要手動更新市值。它和「淨資產」分頁是獨立記錄的,如果你在資產裡也填了投資,記得別重複計算。投資相關決策請自行評估或諮詢專業人士,這個工具只負責追蹤。",
    ...
    guidedTitle: "引導填寫",
    qDone: "完成!",
    qProgress: (step, total) => `第 ${step} / ${total} 題`,
    namePH: "名稱(例如:車貸)",
    amountPH: "輸入或說出金額",
    addThis: "新增此筆",
    listening: "聆聽中…點此停止", micSay: "🎤 用說的",
    heard: (t) => `聽到:「${t}」`,
    back: "上一題", skip: "略過", finish: "完成", next: "下一題",
    allSet: "都填好了 ✦",
  ...
  "zh-CN": {
    ...
    noGoals: "还没有设定目标 → 到「投资与目标」新增",
    goalTarget: (v) => `目标 ${v}`,
    secSavingsRate: "储蓄率",
    savingsRateSub: (m) => `${m} · 收入扣掉支出后存下的比例`,
    monthLabel: "月份",
    totalIncome: "总收入", totalSpending: "总支出", monthlySurplus: "本月结余",
    secBudgetActual: "预算 vs 实际",
    budgetActualSub: "以固定支出当预算,对比本月实际支出(含变动)",
    budgetActualMeta: (actual, budget) => `本月实际 ${actual} / 预算 ${budget}`,
    budgetNone: "尚未设定固定支出",
    over: (v) => `超支 ${v}`, left: (v) => `剩余 ${v}`,
    ...
    goalNoSurplus: "本月无结余",
    investNote: "提醒:这里的投资金额是「现况快照」,你需要手动更新市值。它和「净资产」分页是独立记录的,如果你在资产里也填了投资,记得别重复计算。投资相关决策请自行评估或咨询专业人士,这个工具只负责追踪。",
    etaMonthYear: (d) => `${d.getFullYear()}年${d.getMonth() + 1}月`,
    ...
    guidedTitle: "引导填写",
    qDone: "完成!",
    qProgress: (step, total) => `第 ${step} / ${total} 题`,
    namePH: "名称(例如:车贷)",
    amountPH: "输入或说出金额",
    addThis: "新增此笔",
    listening: "聆听中…点此停止", micSay: "🎤 用说的",
    heard: (t) => `听到:「${t}」`,
    back: "上一题", skip: "跳过", finish: "完成", next: "下一题",
    allSet: "都填好了 ✦",
    ...
    retFields: (cur) => [
      ...
      { k: "monthlySpend", label: "退休后月支出", suffix: cur, hint: "今日币值" },
    ...
    sample: () => ({
      ...
      assets: [
        { id: uid(), label: "活期账户", value: 80000, type: "存款" },
        { id: uid(), label: "证券账户", value: 150000, type: "投资" },
        { id: uid(), label: "应急金", value: 40000, type: "现金" },
      ],
      liabilities: [
  ...
  ja: {
    ...
    tabs: { overview: "概要", cashflow: "収支", networth: "純資産", invest: "投資と目標", retire: "退職" },
    ...
    kMonthlySurplus: "今月の収支",
    surplusSub: (inc, exp) => `収入 ${inc} / 支出 ${exp}`,
    kInvestments: "投資総額",
    holdingsCount: (n) => `${n} 銘柄`,
    secNetWorthTrend: "純資産の推移",
    secAutoRecorded: "更新ごとに自動記録",
    secGoalProgress: "目標の進捗",
    noGoals: "まだ目標がありません → 「投資と目標」で追加",
    goalTarget: (v) => `目標 ${v}`,
    secSavingsRate: "貯蓄率",
    savingsRateSub: (m) => `${m} · 収入から支出を引いて残る割合`,
    monthLabel: "月",
    totalIncome: "総収入", totalSpending: "総支出", monthlySurplus: "今月の収支",
    secBudgetActual: "予算 vs 実績",
    budgetActualSub: "固定支出を予算とし、今月の実績支出(変動含む)と比較",
    budgetActualMeta: (actual, budget) => `実績 ${actual} / 予算 ${budget}`,
    budgetNone: "固定支出が未設定です",
    over: (v) => `${v} 超過`, left: (v) => `${v} 残り`,
    ...
    investNote: "ご注意:ここの投資額は手動更新する「現時点のスナップショット」です。「純資産」タブとは別に記録されるため、資産にも投資を入れている場合は二重計上に注意してください。投資判断はご自身で、または専門家にご相談ください。このツールは記録のみを行います。",
    etaMonthYear: (d) => `${d.getFullYear()}年${d.getMonth() + 1}月`,
    ...
    guidedTitle: "ガイド入力",
    qDone: "完了!",
    qProgress: (step, total) => `質問 ${step} / ${total}`,
    namePH: "名称(例:自動車ローン)",
    amountPH: "金額を入力または発話",
    addThis: "この項目を追加",
    listening: "認識中…タップで停止", micSay: "🎤 話す",
    heard: (t) => `認識:「${t}」`,
    back: "戻る", skip: "スキップ", finish: "完了", next: "次へ",
    allSet: "すべて完了 ✦",
    ...
    retFields: (cur) => [
      ...
      { k: "monthlySpend", label: "退職後の月支出", suffix: cur, hint: "現在の物価" },
    ...
    sample: () => ({
      ...
      assets: [
        { id: uid(), label: "普通預金", value: 1400000, type: "預金" },
        { id: uid(), label: "証券口座", value: 2600000, type: "投資" },
        { id: uid(), label: "緊急資金", value: 750000, type: "現金" },
      ],
      liabilities: [
  ...
  ko: {
    ...
    tabs: { overview: "개요", cashflow: "현금 흐름", networth: "순자산", invest: "투자와 목표", retire: "은퇴" },
    list: { empty: "아직 항목이 없습니다", name: "이름", amount: "금액", add: "추가" },
    kNetWorth: "순자산",
    netWorthSub: (a, d) => `자산 ${a} − 부채 ${d}`,
    kSavingsRate: "이번 달 저축률",
    srExcellent: "훌륭함 ✦", srHealthy: "건전", srImprove: "개선 여지", srOver: "지출이 수입 초과",
    kMonthlySurplus: "이번 달 잔여",
    surplusSub: (inc, exp) => `수입 ${inc} / 지출 ${exp}`,
    kInvestments: "투자 총액",
    holdingsCount: (n) => `${n} 종목`,
    secNetWorthTrend: "순자산 추이",
    secAutoRecorded: "업데이트마다 자동 기록",
    secGoalProgress: "목표 진행률",
    noGoals: "아직 목표가 없습니다 → 「투자와 목표」에서 추가",
    goalTarget: (v) => `목표 ${v}`,
    secSavingsRate: "저축률",
    savingsRateSub: (m) => `${m} · 수입에서 지출을 뺀 후 남기는 비율`,
    monthLabel: "월",
    totalIncome: "총수입", totalSpending: "총지출", monthlySurplus: "이번 달 잔여",
    secBudgetActual: "예산 vs 실제",
    budgetActualSub: "고정 지출을 예산으로, 이번 달 실제 지출(변동 포함)과 비교",
    budgetActualMeta: (actual, budget) => `실제 ${actual} / 예산 ${budget}`,
    budgetNone: "고정 지출이 설정되지 않음",
    over: (v) => `${v} 초과`, left: (v) => `${v} 남음`,
    ...
    investNote: "참고: 여기의 투자 금액은 직접 갱신하는 「현재 스냅샷」입니다. 「순자산」 탭과 별도로 기록되므로, 자산에도 투자를 입력했다면 중복 계산에 주의하세요. 투자 판단은 본인 또는 전문가와 상의하세요. 이 도구는 기록만 합니다.",
    ...
    guidedTitle: "가이드 입력",
    qDone: "완료!",
    qProgress: (step, total) => `질문 ${step} / ${total}`,
    namePH: "이름(예: 자동차 대출)",
    amountPH: "금액 입력 또는 말하기",
    addThis: "이 항목 추가",
    listening: "인식 중…탭하여 중지", micSay: "🎤 말하기",
    heard: (t) => `인식: 「${t}」`,
    back: "이전", skip: "건너뛰기", finish: "완료", next: "다음",
    allSet: "모두 완료 ✦",
    ...
    retFields: (cur) => [
      ...
      { k: "monthlySpend", label: "은퇴 후 월 지출", suffix: cur, hint: "현재 물가" },
    ...
    sample: () => ({
      ...
      assets: [
        { id: uid(), label: "입출금 계좌", value: 15000000, type: "예금" },
        { id: uid(), label: "증권 계좌", value: 28000000, type: "투자" },
        { id: uid(), label: "비상금", value: 8000000, type: "현금" },
      ],
      liabilities: [
...
/* ============================================================================
   HEALTH — strings for the downloadable "Financial Health Check" report.
   Lively / friendly tone, kept in its own export so the report builder can
...
export const HEALTH = {
  en: {
    ...
    scoreLabel: "Overall financial health",
    grades: [
      { min: 85, label: "Thriving!", face: "🌟" },
      { min: 70, label: "Healthy", face: "😄" },
      { min: 55, label: "Doing OK", face: "🙂" },
      { min: 0, label: "Needs love", face: "💪" },
    ],
    dims: { savings: "Saving power", debt: "Debt health", invest: "Investing", cashflow: "Cash flow" },
    metricLabels: { net: "Net worth", rate: "Savings rate", surplus: "Monthly surplus", invest: "Investments" },
    tipsTitle: "A few friendly tips",
    tips: {
      savingsLow: "Try to tuck away a little more each month — even 10% adds up surprisingly fast.",
      savingsGood: "Lovely saving habit — your future self is already saying thanks!",
      debtHigh: "Debt is a big slice of your assets. Chip away at the priciest loans first.",
      debtGood: "Your debt is nicely under control — well balanced!",
      investLow: "A lot is sitting idle. Putting some to work could help it quietly grow.",
      investGood: "Your money is out there working for you. Keep it up!",
      cashLow: "Spending is close to (or above) income. Trimming a category or two helps.",
      cashGood: "You finish each month in the green — lovely breathing room.",
    },
    footer: "Made with Wealth Mosaic",
    privacy: "Your data never left your browser.",
  ...
  "zh-TW": {
    ...
    scoreLabel: "整體財務健康",
    grades: [
      { min: 85, label: "超棒！", face: "🌟" },
      { min: 70, label: "很健康", face: "😄" },
      { min: 55, label: "還不錯", face: "🙂" },
      { min: 0, label: "再加把勁", face: "💪" },
    ],
    dims: { savings: "儲蓄力", debt: "負債健康", invest: "投資佈局", cashflow: "現金流" },
    metricLabels: { net: "淨資產", rate: "儲蓄率", surplus: "每月結餘", invest: "投資總額" },
    tipsTitle: "給你的小建議",
    tips: {
      savingsLow: "試著每個月多存一點點，就算 10% 也會積少成多。",
      savingsGood: "儲蓄習慣很棒，未來的你會感謝現在的你！",
      debtHigh: "負債占資產比重偏高，先從利率最高的開始還吧。",
      debtGood: "負債控制得很好，平衡得相當不錯！",
      investLow: "不少錢還在閒置，讓一部分動起來會更容易長大。",
      investGood: "你的錢正在為你工作，繼續保持！",
      cashLow: "支出快追上（或超過）收入，挑一兩項精簡一下會更輕鬆。",
      cashGood: "每個月都有結餘，喘息空間很舒服。",
    },
    footer: "由 財富拼圖 產生",
    privacy: "你的資料從未離開瀏覽器。",
    download: "下載 PNG",
  ...
  "zh-CN": {
    ...
    grades: [
      { min: 85, label: "超棒！", face: "🌟" },
      { min: 70, label: "很健康", face: "😄" },
      { min: 55, label: "还不错", face: "🙂" },
      { min: 0, label: "再加把劲", face: "💪" },
    ],
    dims: { savings: "储蓄力", debt: "负债健康", invest: "投资布局", cashflow: "现金流" },
    metricLabels: { net: "净资产", rate: "储蓄率", surplus: "每月结余", invest: "投资总额" },
    tipsTitle: "给你的小建议",
    tips: {
      savingsLow: "试着每个月多存一点点，就算 10% 也会积少成多。",
      savingsGood: "储蓄习惯很棒，未来的你会感谢现在的你！",
      debtHigh: "负债占资产比重偏高，先从利率最高的开始还吧。",
      debtGood: "负债控制得很好，平衡得相当不错！",
      investLow: "不少钱还在闲置，让一部分动起来会更容易长大。",
      investGood: "你的钱正在为你工作，继续保持！",
      cashLow: "支出快追上（或超过）收入，挑一两项精简一下会更轻松。",
      cashGood: "每个月都有结余，喘息空间很舒服。",
    },
    footer: "由 财富拼图 生成",
    privacy: "你的数据从未离开浏览器。",
    download: "下载 PNG",
  ...
  ja: {
    btn: "✦ 家計健診",
    title: "家計健診レポート",
    scoreLabel: "総合的な家計の健康度",
    grades: [
      { min: 85, label: "絶好調！", face: "🌟" },
      { min: 70, label: "健康", face: "😄" },
      { min: 55, label: "まずまず", face: "🙂" },
      { min: 0, label: "がんばろう", face: "💪" },
    ],
    dims: { savings: "貯蓄力", debt: "負債の健全性", invest: "投資配分", cashflow: "キャッシュフロー" },
    metricLabels: { net: "純資産", rate: "貯蓄率", surplus: "毎月の収支", invest: "投資総額" },
    tipsTitle: "ちょっとしたヒント",
    tips: {
      savingsLow: "毎月もう少しだけ貯めてみましょう。10%でも積もれば大きいです。",
      savingsGood: "素晴らしい貯蓄習慣です。未来の自分が感謝しています！",
      debtHigh: "負債が資産に占める割合が高めです。金利の高いものから返済を。",
      debtGood: "負債はしっかり管理できています。良いバランスです！",
      investLow: "眠っているお金が多めです。一部を運用に回すと育ちやすくなります。",
      investGood: "お金がしっかり働いてくれています。この調子で！",
      cashLow: "支出が収入に近づいています。1〜2項目を見直すと楽になります。",
      cashGood: "毎月黒字で着地。心地よいゆとりです。",
    },
    footer: "ウエルス・モザイクで作成",
    privacy: "データはブラウザから出ていません。",
    download: "PNGをダウンロード",
    close: "閉じる",
  },
  ko: {
    btn: "✦ 재무 건강검진",
    title: "재무 건강검진 리포트",
    scoreLabel: "전체 재무 건강도",
    grades: [
      { min: 85, label: "최고예요!", face: "🌟" },
      { min: 70, label: "건강해요", face: "😄" },
      { min: 55, label: "괜찮아요", face: "🙂" },
      { min: 0, label: "조금 더 힘내요", face: "💪" },
    ],
    dims: { savings: "저축력", debt: "부채 건전성", invest: "투자 배분", cashflow: "현금 흐름" },
    metricLabels: { net: "순자산", rate: "저축률", surplus: "월 잔여", invest: "투자 총액" },
    tipsTitle: "작은 팁",
    tips: {
      savingsLow: "매달 조금만 더 모아보세요. 10%만 해도 금방 쌓여요.",
      savingsGood: "훌륭한 저축 습관이에요. 미래의 내가 고마워할 거예요!",
      debtHigh: "부채가 자산에서 차지하는 비중이 높아요. 금리 높은 것부터 갚아요.",
      debtGood: "부채를 잘 관리하고 있어요. 균형이 좋아요!",
      investLow: "놀고 있는 돈이 많아요. 일부를 굴리면 더 잘 자라요.",
      investGood: "돈이 당신을 위해 일하고 있어요. 계속 이렇게!",
      cashLow: "지출이 수입에 가까워지고 있어요. 한두 항목을 줄이면 한결 가벼워져요.",
      cashGood: "매달 흑자로 마무리돼요. 여유가 느껴져요.",
    },
    footer: "웰스 모자이크로 생성",
    privacy: "데이터는 브라우저를 벗어나지 않았어요.",
    download: "PNG 다운로드",
    close: "닫기",
  },
};
